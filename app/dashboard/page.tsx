...
export default async function DashboardPage({
  ...
  const { supabase, company, subscription, territory } = await getCompanyContext();
    ...
    <>
      <div className="topbar">
        <div>
          ...
          <div className="muted">
            {territory ? `${territory.centre_postcode} · ${territory.radius_miles} mile radius` : "Territory not configured"}
          </div>
        </div>
      ...
      {params.checkout === "success" && (
        <div className="notice" style={{marginBottom:16}}>
          Payment received. Stripe is confirming the subscription; your access will update automatically.
        </div>
      )}
      ...
      {!active ? (
        <div className="panel locked">
          ...
          <p className="muted" style={{maxWidth:560,margin:"0 auto 22px"}}>
            Your territory is ready. Activate ProjectSignal Pro for £79/month to unlock matched opportunities and daily lead delivery.
          </p>
          ...
          <div className="leads">
            {!leads.length && (
              <div className="panel">
                ...
                <p className="muted">Your account is active. The daily scanner will add projects here when they meet your territory and score settings.</p>
              </div>
            )}
